...
import {
  ...
  // useVerifyPopulation, // disabled — see CENSUS_POPULATION_API.md
...
function getProfileTotalCallsNumeric(profile: any): number | null {
  ...
  const hasF =
    ...
    profile.fire_calls !== "";
  const hasE =
    profile.ems_calls !== null &&
    profile.ems_calls !== undefined &&
    profile.ems_calls !== "";
  if (hasF || hasE)
...
function getPriorFiveUnderwritingRows(
  underwriting: any[],
  targetYear: string,
): any[] | null {
  ...
  const idx = asc.findIndex(
    (r) => String(r.underwriting_year) === String(targetYear),
  );
...
function describeUnderwritingRowCalcGaps(
  ...
  if (!opts?.isRenewal) {
    ...
    if (
      row.total_loss_lae !== null &&
      row.total_loss_lae !== undefined &&
      row.total_loss_lae !== ""
    ) {
    ...
    if (!Number.isFinite(totalLossLae))
      gaps.push("total loss/LAE (valid number)");

    let lossRatioPct: number | null = null;
    if (
      row.loss_ratio !== null &&
      row.loss_ratio !== undefined &&
      row.loss_ratio !== ""
    ) {
    ...
    if (lossRatioPct === null || !Number.isFinite(lossRatioPct)) {
      gaps.push(
        "loss ratio (enter a value or ensure premium + losses/LAE allow calculation)",
      );
    }
  }
...
function lastNonEmptyUnderwritingYear(
  rows: { underwriting_year?: string }[],
): string {
  ...
  for (const uw of rows) {
    ...
    const prem =
      Number.isFinite(tp) && tp > 0
        ? tp
        : (parseFloat(uw.vfbl) || 0) + (parseFloat(uw.wc) || 0);
    totalPremium += prem;
...
export default function AnalysisDetail() {
  ...
  // const verifyPopulation = useVerifyPopulation(); // disabled — see CENSUS_POPULATION_API.md
    ...
    return (
      <Box p={4}>
        <Alert severity="warning" sx={{ mb: 2 }}>
          Missing or invalid <code>company_id</code>. Open this page from the
          analysis list, or add <code>?company_id=</code> to the URL (subscribed
          company for this fire department).
        </Alert>
  ...
  const formatTotalLossLaeCell = (uw: any) => {
    ...
    if (stored !== null && stored !== undefined && stored !== "") {
      const n =
        typeof stored === "string" ? parseFloat(stored) : Number(stored);
    ...
    return formatCurrency(
      (Number.isNaN(losses) ? 0 : losses) + (Number.isNaN(lae) ? 0 : lae),
    );
  };
        ...
        if (code === "MISSING_5_YEAR_DATA") {
          const existingRows: UnderwritingRowForm[] = (underwriting || []).map(
            (u: any) => ({
              ...
              number_of_claims:
                u.number_of_claims != null ? Number(u.number_of_claims) : "",
          ...
          message:
            code === "MISSING_5_YEAR_DATA"
              ...
              : message.includes("Missing:")
                ? message
                : "Profile is missing required fields. Please complete the fields below and try again.",
        });
        return;
      }
  ...
  const underwritingRowsWithYear = underwritingRows.filter(
    (r) => String(r.underwriting_year ?? "").trim() !== "",
  );
  const duplicateYears = (() => {
    const years = underwritingRowsWithYear.map((r) =>
      String(r.underwriting_year).trim(),
    );
  ...
  const handleAddUnderwritingRow = () => {
    setUnderwritingRows((prev) => {
      const suggested = suggestPriorYearUnderwritingYear(
        lastNonEmptyUnderwritingYear(prev),
      );
      return [
        ...prev,
        {
  ...
  const handleSaveUnderwriting = async () => {
    const filled = underwritingRows.filter(
      (r) => String(r.underwriting_year ?? "").trim() !== "",
    );
    if (filled.length === 0) {
    ...
    if (hasDuplicateYears) {
      toast.error(
        "Same year cannot be entered twice. Please remove or change duplicate years.",
      );
      return;
    }
    const rows = filled.map((r) => ({
      ...
      number_of_claims:
        r.number_of_claims === "" ? undefined : Number(r.number_of_claims),
    ...
    try {
      ...
      toast.error(
        e?.response?.data?.message || "Failed to save underwriting data",
      );
    }
  };
  ...
  const priorFiveForTotals =
    underwriting && currentYear
      ? getPriorFiveUnderwritingRows(underwriting, currentYear)
      : null;
  ...
  const underwritingRowCompleteForCalc = (
    ...
    if (
      opts?.requireCategory &&
      !String(row?.type ?? row?.category ?? "").trim()
    )
      return false;
  ...
  const getMissingProfileForCalc = (): string[] => {
    ...
    if (
      profile.motorized_racing_team === null ||
      profile.motorized_racing_team === undefined
    ) {
    ...
    if (
      !hasNumeric(profile.square_miles) ||
      Number(profile.square_miles) <= 0
    ) {
    ...
    if (!hasNumeric(profile.hs_officers))
      missing.push("# of H&S Officers (0 if none)");
    ...
    if (
      hasNumeric(profile.square_miles) &&
      sqMi > 0 &&
      hasNumeric(profile.population)
    ) {
      const density = pop / sqMi;
      if (!Number.isFinite(density))
        missing.push("Density (population ÷ sq mi)");
    }
  ...
  const priorFiveForCalc =
    underwriting && currentYear
      ? getPriorFiveUnderwritingRows(underwriting, currentYear)
      : null;
  const hasPriorFiveYearWindow =
    priorFiveForCalc !== null && priorFiveForCalc.length === 5;
  const missingUnderwritingData =
    hasPriorFiveYearWindow && currentYearData
      ? priorFiveForCalc!.some(
          (row: any) =>
            !underwritingRowCompleteForCalc(row, { requireCategory: true }),
        ) ||
  ...
  const calculateDisabledTooltipContent = !canCalculateAnalysis ? (
    <Box component="div" sx={{ maxWidth: 400, py: 0.25 }}>
      ...
      {!currentYear && (
        ...
        >
          • <strong>Underwriting:</strong> add at least one year (Edit
          underwriting).
        </Typography>
      )}
      {missingProfileFields.length > 0 && (
      ...
      {currentYear && !hasPriorFiveYearWindow && (
        <Typography
          ...
          sx={{ display: "block", mb: 0.75 }}
        >
          • <strong>Underwriting years:</strong> need the latest renewal row
          plus five periods immediately before it (six rows total, contiguous).
          You currently have {underwriting?.length ?? 0} row(s). Add older years
        ...
        priorFiveForCalc && (
          <Box>
            ...
            <Typography
              ...
              component="div"
              color="inherit"
              sx={{ display: "block", mb: 0.35, opacity: 0.9 }}
            >
              # Claims is optional. The latest year only needs an underwriting
              year — premium / Losses / LAE / loss ratio / Category are all
              optional on it. The five prior years still require premium,
              Losses, LAE, a derivable loss ratio, and Category (used in the
              5-year totals).
            </Typography>
            <Box component="ul" sx={{ m: 0, pl: 2.25, mb: 0 }}>
  ...
  return (
    <Box sx={{ p: 3 }}>
              ...
              {profile?.customer_since ?? fire_department?.customer_since
                ? new Date(
                    (profile?.customer_since ??
                      fire_department?.customer_since) as string,
                  ).toLocaleDateString()
                : "-"}
            </Typography>
          </Grid>
          ...
          <Grid container spacing={1} columnSpacing={{ xs: 1, sm: 2 }}>
            ...
            <Grid item xs={12} sm={8} md={4}>
              ...
              >
                ...
                {/* US Census county population UI disabled — per fire dept population from form/profile only.
                    See CENSUS_POPULATION_API.md to re-enable verify-population API and buttons below.
                {(() => {
            ...
            <Grid item xs={6} sm={4} md={2}>
              ...
              <Typography variant="body2" fontWeight={600}>
                {formatProfileTotalCalls(profile)}
              </Typography>
            </Grid>
            ...
            <Grid item xs={6} sm={4} md={2}>
              ...
              <Typography variant="body2" fontWeight={600}>
                {profile.hs_officers != null && profile.hs_officers !== ""
                  ? profile.hs_officers
                  : "-"}
              </Typography>
            </Grid>
      ...
      <Paper elevation={2} sx={{ mb: 3 }}>
        <Box sx={{ p: 2, borderBottom: 1, borderColor: "divider" }}>
          ...
          >
            ...
            <Stack direction="row" spacing={1}>
              <Button
                ...
                color="primary"
                size="small"
                onClick={handleOpenUnderwritingDialog}
              >
                Edit
              </Button>
          ...
          {currentYear && (
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
              Latest underwriting year on file for this company:{" "}
              <strong>{currentYear}</strong>. Calculate Analysis uses this year
              until you add a newer year row (Edit underwriting).
            </Typography>
          )}
          {!currentYear && (
                  ...
                  {underwriting.map((uw: any, index: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          {underwritingRowCategoryLabel(uw)}
                        </TableCell>
                        <TableCell align="right">
                          {formatCurrency(uw.vfbl)}
                        </TableCell>
                        <TableCell align="right">
                          {formatCurrency(uw.wc)}
                        </TableCell>
                        <TableCell align="right">
                          {formatCurrency(uw.total_premium)}
                        </TableCell>
                        <TableCell align="right">
                          {formatCurrency(uw.losses)}
                        </TableCell>
                        <TableCell align="right">
                          {formatCurrency(uw.lae)}
                        </TableCell>
                        <TableCell align="right">
                          {formatTotalLossLaeCell(uw)}
                        </TableCell>
                        <TableCell align="right">
        ...
        >
          5 Yr Totals sums the{" "}
          <strong>five underwriting periods immediately before</strong> the
          latest year (same as the worksheet).{" "}
          <strong>Calculate Analysis</strong> uses this row for loss ratio
          points and for claims per $100k premium (total claims ÷ total premium
          × 100,000). Individual year rows do not store or show points (like the
          worksheet). Only the 5 Yr Totals row shows{" "}
          <strong>loss-ratio points</strong> from the last calculation when it
          matches the latest year. The <strong>Category</strong> column is
          stored per year on each underwriting row (FDM / FDI / FPI); set it in{" "}
          <strong>Edit</strong> or it is written on the latest year when you run{" "}
          <strong>Calculate Analysis</strong>.
        </Typography>
        ...
        {latestResult && (
          ...
          >
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              After you run <strong>Calculate Analysis</strong>, this section
              shows how points were built from the <strong>5-yr totals</strong>{" "}
              row (loss ratio and frequency), the fire department profile, and
              the management penalty, then the total and assigned category (FDM
              / FDI / FPI).
            </Typography>

            <Box component="ul" sx={{ pl: 2.5, mb: 2, mt: 0 }}>
              <Typography component="li" variant="body2" sx={{ mb: 0.75 }}>
                <strong>Loss ratio points</strong> (
                {latestResult.loss_ratio_points ?? 0}):{" "}
                {calcLossRatioPct != null ? (
                  <>
                    combined loss ratio over the{" "}
                    <strong>five years before</strong> the renewal year{" "}
                    <strong>{calcLossRatioPct.toFixed(2)}%</strong> (total loss
                    + LAE ÷ total premium on that 5-yr row), looked up in the
                    loss-ratio points table.
                  </>
                ) : (
              ...
              <Typography component="li" variant="body2" sx={{ mb: 0.75 }}>
                <strong>Density points</strong> (
                {latestResult.density_points ?? 0}):{" "}
                {calcDensity != null ? (
                  <>
                    density <strong>{calcDensity.toLocaleString()}</strong>{" "}
                    (rounded population ÷ square miles on profile), looked up in
                    the density points table.
                  </>
                ) : (
                  "—"
                )}
              ...
              <Typography component="li" variant="body2" sx={{ mb: 0.75 }}>
                <strong>Call volume points</strong> (
                {latestResult.call_volume_points ?? 0}):{" "}
                {calcTotalCalls != null ? (
                  <>
                    <strong>{calcTotalCalls.toLocaleString()}</strong> total
                    calls (fire + EMS on profile), looked up in the call-volume
                    table.
                  </>
                ) : (
                  "—"
              ...
              <Typography component="li" variant="body2" sx={{ mb: 0.75 }}>
                <strong>Frequency points</strong> (
                {latestResult.frequency_factor_points ?? 0}):{" "}
                {calcClaimsPer100k != null ? (
                  <>
                    claims per $100k premium{" "}
                    <strong>{calcClaimsPer100k.toFixed(2)}</strong> (total #
                    claims ÷ total premium × 100,000 on the 5-yr totals row),
                    looked up in the frequency table.
                  </>
                ) : (
                  "—"
                )}
              ...
              <Typography component="li" variant="body2" sx={{ mb: 0.75 }}>
                <strong>Safety / H&amp;S / racing</strong>: safety +1 if Yes, −2
                if No (unset in profile keeps Calculate disabled); H&amp;S
                officers +1 if count is greater than zero, −2 if zero (blank
                keeps Calculate disabled); motorized racing −2 if the team is
                present (checkbox or race count), 0 if not.
              </Typography>
              <Typography component="li" variant="body2">
                ...
                . If the sum of all point lines would exceed <strong>31</strong>
                , the total is capped at <strong>31</strong>; category is then
                based on that capped total (the <strong>26–31</strong> band is{" "}
                <strong>FPI</strong>).
              </Typography>
            </Box>
              ...
              <Grid item xs={12}>
                ...
                >
                  ...
                  <Typography variant="body1" sx={{ mt: 1 }}>
                    Assigned company:{" "}
                    {latestResult.assignedCompany?.Company_Name || "—"}
                  </Typography>
        ...
        <DialogContent>
          ...
          {dataEntryDialog.code === "MISSING_PROFILE" && (
            <Stack spacing={2} sx={{ mt: 1 }}>
                ...
                control={
                  <Checkbox
                    ...
                    onChange={(e) =>
                      setProfileForm((p) => ({
                        ...p,
                        motorized_racing_team: e.target.checked,
                      }))
                    }
                  />
                }
              ...
              >
                ...
                >
                  Required to run Calculate Analysis. Leave unset only if you
                  have not decided yet (analysis stays disabled).
                </Typography>
              </FormControl>
              <TextField
          ...
          {dataEntryDialog.code === "MISSING_5_YEAR_DATA" && (
            <Stack spacing={2} sx={{ mt: 1 }}>
              {hasDuplicateYears && (
                <Alert severity="error">
                  Same year cannot be entered twice. Please use a unique year
                  (e.g. 2024-2025) for each row.
                </Alert>
              )}
              <Typography
                variant="caption"
                ...
                sx={{ display: "block" }}
              >
                New rows use the next older period from the row above (e.g.{" "}
                <strong>2022-2021</strong> → <strong>2021-2020</strong>). Tab
                out of Year to fill the row below if it is still empty.
              </Typography>
              {underwritingRows.map((row, idx) => (
                <Paper
                  ...
                  sx={{
                    p: 2,
                    borderColor: duplicateYears.has(
                      String(row.underwriting_year).trim(),
                    )
                      ? "error.main"
                      : undefined,
                  }}
                    ...
                    <TextField
                      ...
                      onChange={(e) =>
                        setUnderwritingRows((prev) =>
                          prev.map((r, i) =>
                            i === idx
                              ? { ...r, underwriting_year: e.target.value }
                              : r,
                          ),
                        )
                      }
                      onBlur={() => {
                        ...
                        setUnderwritingRows((prev) => {
                          ...
                          if (
                            String(nextRow?.underwriting_year ?? "").trim() !==
                            ""
                          )
                            return prev;
                          return prev.map((r, i) =>
                            i === idx + 1
                              ? { ...r, underwriting_year: suggested }
                              : r,
                          );
                        });
                      }}
                      error={duplicateYears.has(
                        String(row.underwriting_year).trim(),
                      )}
                      sx={{ maxWidth: 160 }}
                    />
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                      <FormControl fullWidth size="small">
                        <InputLabel id={`uw-category-${idx}`}>
                          Category (FDM / FDI / FPI)
                        </InputLabel>
                        <Select
              ...
              <Button
                variant="contained"
                color="primary"
                onClick={handleAddUnderwritingRow}
                fullWidth
              >
                Add row
              </Button>
            ...
            disabled={
              dataEntryDialog.code === "MISSING_PROFILE"
                ...
                : bulkUpsertUnderwriting.isPending ||
                  (dataEntryDialog.code === "MISSING_5_YEAR_DATA" &&
                    (hasDuplicateYears ||
                      underwritingRowsWithYear.length === 0))
            }
          >
            {dataEntryDialog.code === "MISSING_PROFILE"
              ? updateProfile.isPending
